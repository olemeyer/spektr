...
_SPEKTR_PKG = os.path.dirname(os.path.abspath(__file__))
...
def format_record_rich(record: LogRecord) -> None:
    ...
    # structured data (context + call-site)
    merged = {**record.context, **record.data}
    if merged:
        line.append("  ")
...
def format_trace_json(root: SpanData) -> None:
    def _serialize(span: SpanData) -> dict:
        ...
        if span.data:
            d["attributes"] = {k: str(v) if not isinstance(v, (int, float, bool)) else v for k, v in span.data.items()}
        if span.error:
